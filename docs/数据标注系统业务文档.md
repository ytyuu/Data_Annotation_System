...
本文档根据项目流程图和 `docs/数据标注数据库设计.sql` 整理，用于说明数据标注系统的角色、业务对象、主流程和数据库状态含义。后续涉及业务流程、接口、页面或数据库变更时，应优先参考本文档。
...
- 草稿状态的数据集支持手动输入和 CSV 文件两种导入方式，导入后的数据项初始状态均为 `pending`。
- 手动输入按行生成文本数据项，空行忽略，单次最多导入 500 条。
- CSV 文件必须为 UTF-8 编码且无表头，每个物理行生成一条 `content_type = 'text'` 的数据项；首行直接作为数据导入，空行忽略。
- CSV 导入不限制数据项条数，当前采用同步处理，单个文件最大 100 MB，后端每 1000 条执行一次批量写入并在同一事务中完成；任一行格式错误时整次导入回滚。
- CSV 导入按物理行读取原始内容，不解析逗号或双引号语法；英文逗号、双引号及首尾空格均原样保留，仅移除行末换行符和文件首行的 UTF-8 BOM。当前不支持单条数据跨越多个物理行。
- CSV 导入会在 `metadata` 中记录 `import_source`、`import_filename` 和 `import_line_no`，用于追踪文件来源及原始行号。
