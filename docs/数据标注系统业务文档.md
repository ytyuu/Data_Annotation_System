...
- CSV 文件必须为 UTF-8 编码、无表头且只有一列，每个物理行生成一条 `content_type = 'text'` 的数据项；首行直接作为数据导入，空行忽略。
- CSV 导入不限制数据项条数，当前采用同步处理，单个文件最大 100 MB，后端每 1000 条执行一次批量写入并在同一事务中完成；任一行格式错误时整次导入回滚。
- 未使用双引号包裹的英文逗号会被识别为多列并拒绝导入；内容包含英文逗号时需使用 CSV 双引号包裹，内容中的双引号使用两个连续双引号转义。当前不支持单条数据跨越多个物理行。
