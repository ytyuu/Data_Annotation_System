...
```mermaid
flowchart LR
    P["提供方页面"] -->|"创建批次、审核结果"| A["Kotlin API"]
    W["TypeScript AI Worker"] -->|"领取数据、上传结果"| A
...
- `apps/web`：批次创建、进度查看、结果审核和抽检操作。
- `apps/api`：身份校验、数据锁定、租约、结果校验、状态流转、计数和事务。
- `apps/ai-worker`：读取配置、构建 Prompt、调用模型、校验模型输出、重试和上传。
